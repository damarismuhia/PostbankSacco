package com.android.postbanksacco.ui.fragments.auth.login
...
import com.android.postbanksacco.databinding.FragmentLoginBinding
import com.android.postbanksacco.ui.activities.HomeActivity
...
@AndroidEntryPoint
class LoginFragment : BaseAuthFragment<FragmentLoginBinding, AuthViewModel>(FragmentLoginBinding::inflate) {
    ...
    private fun initUI() {
        binding.apply {
            tvTitle.text =  if (readItemFromPref(CacheKeys.FIRST_NAME).isNotEmpty()){
                "${getGreetings(requireContext())} ${readItemFromPref(CacheKeys.FIRST_NAME).capitalizeWords()},"
            }else {
                getGreetings(requireContext())
            }
            ...
            btnUseFingerprint.setOnClickListener{
                EncryptedPref.writePreference(requireContext(),CacheKeys.fingerPrintClickedToEnroll,"true")
        ...
        CoroutineScope(Dispatchers.Main).launch {
            ...
            if (progressDialog.isShowing) {
                ...
                if (simulateLoginResponse(phone,mConfirmPin!!)){
                    ...
                    if (enrollFingerPrint) {
                        EncryptedPref.writePreference(requireContext(),CacheKeys.setIsFingerPrintEnrolled,"true")
                        if (this@LoginFragment::cryptoObjectData.isInitialized){
                            EncryptedPref.writePreference(requireContext(),CacheKeys.secureFinger,cryptoObjectData)
                        }
                    }
                   navigateToDashboard()
                }else{
